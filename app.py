...
st.title("AMR-Intel: Integrated Genome-Based AMR Prediction App")
...
def run_full_amr_pipeline(input_fasta, sample_name):
    ...
    bash_command = (
        ...
        f"conda run -n rgi6_env prodigal "
        ...
        f"conda run -n rgi6_env rgi main "
        ...
        f"conda run -n rgi6_env amrfinder "
        f"-p {wsl_protein_clean_faa} "
        f"-o {wsl_amrfinder_output} && "

        f"conda run -n abricate_env abricate "
        f"--db resfinder "
        f"{wsl_cleaned_fasta} "
...
def download_ncbi_genome(accession, sample_name):
    ...
    bash_command = (
        f"source ~/miniconda3/etc/profile.d/conda.sh && "
        f"conda run -n rgi6_env datasets download genome accession {accession} "
        f"--include genome "
...
def assemble_wgs_reads(r1_path, r2_path, sample_name):
    ...
    bash_command = (
        f"source ~/miniconda3/etc/profile.d/conda.sh && "
        f"conda run -n rgi6_env shovill "
        f"--R1 {wsl_r1} "
        f"--R2 {wsl_r2} "
...
st.sidebar.write("Required tools inside WSL:")
st.sidebar.write("1. `rgi6_env` for Prodigal, CARD-RGI, AMRFinderPlus, NCBI Datasets CLI and Shovill")
st.sidebar.write("2. `abricate_env` for ABRicate")
st.sidebar.write("3. CARD database loaded in `D:/AMR_App/card_database/localDB`")
